...
public class LabelResolver {
    ...
    private static Properties loadLabels(final String sdkVersion, final Path sdkPath,
            final String language) {
        final Properties props = new Properties();
        if (sdkVersion == null || sdkPath == null) {
            return props;
        }
        final String filename = "rule_" + language + ".xml";
        try (InputStream stream = SdkResourceLoader.getResourceAsStream(
                sdkVersion, SdkConstants.SdkResource.TRANSLATIONS, filename, sdkPath)) {
            props.loadFromXML(stream);
        } catch (final Exception e) {
            logger.debug("Could not load rule labels from {}: {}", filename, e.getMessage());
        }
        return props;
    }
}
